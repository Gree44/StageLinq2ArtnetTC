...
exports.createDiscoveryRequest = createDiscoveryRequest;
exports.createDiscoveryResponse = createDiscoveryResponse;
exports.parseDiscoveryResponse = parseDiscoveryResponse;
exports.isDiscoveryRequest = isDiscoveryRequest;
...
function createDiscoveryRequest() {
    ...
    const magic = new TextEncoder().encode(types_1.EAAS_MAGIC);
    buffer.set(magic, 0);
    buffer[4] = 0x01; // Version
    buffer[5] = types_1.EAAS_REQUEST_TYPE;
    return buffer;
}
/**
 ...
 */
function createDiscoveryResponse(token, hostname, url, softwareVersion) {
    ...
    // Version and type
    buffer[offset++] = 0x01;
    buffer[offset++] = types_1.EAAS_RESPONSE_TYPE;
    // Token (16 bytes)
 ...
 */
function parseDiscoveryResponse(data, remoteAddress) {
    if (data.length < 22) {
        return null; // Too short to be valid
    }
    // Check magic bytes
    const magic = new TextDecoder().decode(data.slice(0, 4));
    if (magic !== types_1.EAAS_MAGIC) {
        return null;
    }
    // Check version and type
    if (data[4] !== 0x01 || data[5] !== types_1.EAAS_RESPONSE_TYPE) {
        return null;
    }
    let offset = 6;
    // Read token (16 bytes)
    ...
    offset += 4;
    if (offset + hostnameLen > data.length)
        return null;
    ...
    let grpcPort = types_1.EAAS_GRPC_PORT;
    let httpPort = types_1.EAAS_HTTP_PORT;
 ...
 */
function isDiscoveryRequest(data) {
    if (data.length !== 6) {
        return false;
    }
    const magic = new TextDecoder().decode(data.slice(0, 4));
    return magic === types_1.EAAS_MAGIC && data[4] === 0x01 && data[5] === types_1.EAAS_REQUEST_TYPE;
}
/**
