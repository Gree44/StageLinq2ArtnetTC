...
class Player extends events_1.EventEmitter {
    ...
    constructor(options) {
        super();
        this.player = 0; // Player number as reported by the device.
        this.masterTempo = 0; // Current master tempo BPM
        this.masterStatus = false; // If this device has the master tempo
        this.decks = new Map();
        this.lastTrackNetworkPath = new Map();
        this.queue = {};
     ...
     */
    handleUpdate(data) {
        LogEmitter_1.Logger.debug(`data: ${JSON.stringify(data, null, 2)}`);
        ...
        if (isNewTrack && isSongLoaded) {
            LogEmitter_1.Logger.debug(`Replacing state ${layer}`);
            this.decks.set(layer, data);
        }
        else {
            LogEmitter_1.Logger.debug(`Updating state ${layer}`);
    ...
    getSourceAndTrackPath(p_path) {
        if (!p_path || p_path.length === 0)
            return { source: '', trackPath: '', trackPathAbsolute: '' };
