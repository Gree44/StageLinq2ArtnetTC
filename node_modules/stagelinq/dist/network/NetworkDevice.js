...
class NetworkDevice {
    ...
    async connect() {
        ...
        LogEmitter_1.Logger.debug(`Attempting to connect to ${info.address}:${info.port}`);
        ...
        while (ctx.isEOF() === false) {
            ...
            switch (id) {
                ...
                case types_1.MessageId.ServicesRequest:
                    ...
                    break;
                default:
                    assert_1.strict.fail(`NetworkDevice Unhandled message id '${id}'`);
            }
        }
    }
    ///////////////////////////////////////////////////////////////////////////
    ...
    async addSource(p_sourceName, p_localDbPath, p_localAlbumArtPath) {
        ...
        const db = new better_sqlite3_1.default(p_localDbPath);
    ...
    async dumpAlbumArt(p_sourceName) {
        ...
        LogEmitter_1.Logger.info(`dumped ${result.length} albums arts in '${path}'`);
    }
    // Database helpers
    ...
    getSourceAndTrackFromNetworkPath(p_path) {
        const parsed = (0, trackPath_1.parseNetworkPath)(p_path);
        if (!parsed) {
            return null;
        }
        return {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            while (true) {
                ...
                if (Object.keys(this.servicePorts).length > 3) {
                    LogEmitter_1.Logger.debug(`Discovered the following services on ${this.address}:${this.port}`);
                    for (const [name, port] of Object.entries(this.servicePorts)) {
                        LogEmitter_1.Logger.debug(`\tport: ${port} => ${name}`);
                    }
                    resolve();
                    break;
                }
